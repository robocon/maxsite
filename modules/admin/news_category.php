												...
												if($_GET['op'] == ""){
													...
													<form action="?name=admin&file=news_category&op=newscat_del&action=multidel" name="myform" method="post">
														<table width="100%" cellspacing="2" cellpadding="1" >
															...
															$newsCatRows = $db->rows($query);
															$CATCOUNT = 0 ;
															while ($newsCat = mysql_fetch_array($query)){
																$newsNum = $db->num_rows(TB_NEWS,"id"," category=".$newsCat['id']." ");

																$CATCOUNT++;
																//กำหนดการเปลี่ยนลำดับขึ้น
																$SETSORT_UP = $newsCat['sort']-1;
																if($CATCOUNT == "1"){
																	$SETSORT_UP = "1" ;
																}
																//กำหนดการเปลี่ยนลำดับลง
																$SETSORT_DOWN = $newsCat['sort']+1;
																if($CATCOUNT == $newsCatRows){
																	$SETSORT_DOWN = $newsCat['sort'] ;
																}

																?>
																<tr>
																	<td width="44">
																		<a href="?name=admin&file=news_category&op=newscat_edit&id=<?php echo $newsCat['id'];?>"><img src="images/admin/edit.gif" border="0" alt="แก้ไข" ></a>
																		<a href="javascript:Confirm('?name=admin&file=news_category&op=newscat_del&id=<?php echo $newsCat['id'];?>','คุณมั่นใจในการลบหมวดหมู่นี้ ?');"><img src="images/admin/trash.gif"  border="0" alt="ลบ" ></a>
																	</td>
																	<td><?php echo $newsCat['category_name'];?></td>
																	<td align="center" width="50" ><?php echo $newsNum ;?></td>
																	<td align="center" width="50"><A HREF="?name=admin&file=news_category&op=newscat_edit&action=sort&setsort=<?php echo $SETSORT_UP ;?>&move=up&id=<?php echo $newsCat['id'];?>"><IMG SRC="images/icon/arrow_up.gif"  BORDER="0" ALT="เลื่อนขึ้น"></A>&nbsp;&nbsp;&nbsp;<A HREF="?name=admin&file=news_category&op=newscat_edit&action=sort&setsort=<?php echo $SETSORT_DOWN ;?>&move=down&id=<?php echo $newsCat['id'];?>"><IMG SRC="images/icon/arrow_down.gif"  BORDER="0" ALT="เลื่อนลง"></A></td>
																		<td valign="top" align="center" width="40"><input type="checkbox" name="list[]" value="<?php echo $newsCat['id'];?>"></td>
																	</tr>
																	<TR>
														...
														if(CheckLevel($_SESSION['admin_user'],$_GET['op'])){
															...
															$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
															$query = $db->select_query("SELECT sort FROM ".TB_NEWS_CAT." ORDER BY sort DESC ");
															$newsCat = mysql_fetch_array($query);
															$SORTID = $newsCat['sort']+1 ;
																...
																<B>ชื่อหมวดหมู่ :</B><BR>
																	<INPUT TYPE="text" NAME="CATEGORY" size="40">
																		...
																		if(CheckLevel($_SESSION['admin_user'],$_GET['op'])){
																			...
																			if($_GET['move'] == "up"){
																				$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
																				$query = "UPDATE ".TB_NEWS_CAT." SET sort = sort+1 WHERE sort = '".$_GET['setsort']."' ";
																				mysql_query ( $query ) or sql_error ( "db-query",mysql_error() );
																				$db->closedb ();

																				$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
																				$query = "UPDATE ".TB_NEWS_CAT." SET sort = '".$_GET['setsort']."' WHERE id = '".$_GET['id']."' ";
																				mysql_query ( $query ) or sql_error ( "db-query",mysql_error() );
																				$db->closedb ();
																			}
																			if($_GET['move'] == "down"){
																		...
																		if(CheckLevel($_SESSION['admin_user'],$_GET['op'])){
																			//ดึงค่า
																			$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
																			$query = $db->select_query("SELECT * FROM ".TB_NEWS_CAT." WHERE id='".$_GET['id']."' ");
																			$newsCat = $db->fetch($query);
																			$db->closedb ();
																			?>
																			<FORM METHOD=POST ACTION="?name=admin&file=news_category&op=newscat_edit&action=edit&id=<?=$_GET['id'];?>">
																				<B>ชื่อหมวดหมู่ :</B><BR>
																					<INPUT TYPE="text" NAME="CATEGORY" size="40" value="<?=$newsCat['category_name'];?>">
																						<BR><BR>
