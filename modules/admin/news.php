												...
												if( $_GET['op'] === false ){
													...
													<form action="?name=admin&file=news&op=news_del&action=multidel" name="myform" method="post">
														<table width="100%" cellspacing="2" cellpadding="1" >
															...
															<?php
															$query = $db->select_query("SELECT * FROM ".TB_NEWS." ORDER BY id DESC LIMIT $goto, $limit ");
															while($news = $db->fetch($query)){
																$qCategory = $db->select_query("SELECT * FROM ".TB_NEWS_CAT." WHERE id='".$news['category']."' ");
																$category = $db->fetch($qCategory);
																//Comment Icon
																...
																<tr>
																	<td width="44">
																		<a href="?name=admin&file=news&op=news_edit&id=<?php echo $news['id'];?>"><img src="images/admin/edit.gif" border="0" alt="แก้ไข" ></a>
																		<a href="javascript:Confirm('?name=admin&file=news&op=news_del&id=<?php echo $news['id'];?>&prefix=<?php echo $news['post_date'];?>','คุณมั่นใจในการลบหัวข้อนี้ ?');"><img src="images/admin/trash.gif"  border="0" alt="ลบ" ></a>
																	</td>
																	<td><A HREF="?name=news&file=readnews&id=<?php echo $news['id'];?>" target="_blank"><?php echo $news['topic'];?></A><?=$CommentIcon;?></td>
																	<td ><CENTER><?php echo ThaiTimeConvert($news['post_date'],'','');?></CENTER></td>
																	<td align="center">
																		<?php if($category['category_name']){ //หากมีหมวดแสดงรูป ?>
																			...
																			<td valign="top" align="center" width="40"><input type="checkbox" name="list[]" value="<?php echo $news['id'];?>"></td>
																		</tr>
																		<TR>
																															...
																															<BR>
																																<INPUT TYPE="checkbox" NAME="ENABLE_COMMENT" VALUE="1"> อนุญาติให้มีการแสดงความคิดเห็น
																																	...
																																	if(CheckLevel($_SESSION['admin_user'],$_GET['op'])){
																																		...
																																		$FileNewsTopic = "newsdata/".$news['post_date'].".txt";
																																		...
																																		<FORM NAME="myform" METHOD=POST ACTION="?name=admin&file=news&op=news_edit&action=edit&id=<?=$_GET['id'];?>" enctype="multipart/form-data">
																																			<B>หัวข้อ :</B><BR>
																																				<INPUT TYPE="text" NAME="TOPIC" size="50" value="<?=$news['topic'];?>">
																																					<BR><BR>
																																						<B>รายละเอียดย่อสั้นๆ :</B><BR>
																																							<INPUT TYPE="text" NAME="HEADLINE" size="100" value="<?=$news['headline'];?>">
																																								<BR><BR>
																																									<B>หมวดหมู่ :</B><BR>
																																										<SELECT NAME="CATEGORY">
																																											<?php
																																											...
																																											while ($category = $db->fetch($query)){
																																												...
																																												if($category['id'] == $news['category']){echo " Selected";}
																																										...
																																										<BR><BR>
																																											<B>รูปไอคอนข่าวสาร : </B><BR>
																																												<IMG name="view01" SRC="newsicon/<?=$news['post_date'];?>.jpg" <?php echo " WIDTH=\""._INEWS_W."\" HEIGHT=\""._INEWS_H."\" ";?> BORDER="0" ><BR>
																																													<input type="file" name="FILE" onpropertychange="view01.src=FILE.value;" style="width=250;"><BR>
																																														รูปเป็นไฟล์ .jpg  .jpeg ขนาด <?php echo _INEWS_W." x "._INEWS_H ;?> Pixels เท่านั้น (หากรูปใหญ่จะย่อให้อัตโนมัติ)
																																														<BR><BR>
																																															<B>เนื้อหา :</B><BR>
																																																...
																																																?>
																																																<BR>
																																																	<INPUT TYPE="checkbox" NAME="ENABLE_COMMENT" VALUE="1" <?php if($news['enable_comment']){echo " Checked";};?>> อนุญาติให้มีการแสดงความคิดเห็น
																																																		<BR>
																																																		...
																																																		if(CheckLevel($_SESSION['admin_user'],$_GET['op'])){
																																																			while(list($key, $value) = each ($_POST['list'])){
																																																				$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
																																																				$query = $db->select_query("SELECT * FROM ".TB_NEWS." WHERE id='".$value."' ");
																																																				$news = $db->fetch($query);
																																																				$db->del(TB_NEWS," id='".$value."' ");
																																																				$db->closedb ();
																																																				@unlink("newsdata/".$news['post_date'].".txt");
																																																				@unlink("newsicon/".$news['post_date'].".jpg");
																																																			}
																																																			$ProcessOutput = "<BR><BR>";
																																																			$ProcessOutput .= "<CENTER><A HREF=\"?name=admin&file=main\"><IMG SRC=\"images/icon/login-welcome.gif\" BORDER=\"0\"></A><BR><BR>";
