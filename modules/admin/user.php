													...
													if( !isset($_GET['op']) ){
														...
														<form action="?name=admin&file=user&op=admin_del&action=multidel" name="myform" method="post">
															<table width="100%" cellspacing="2" cellpadding="1" >
																...
																<?php
																$query = $db->select_query("SELECT * FROM ".TB_ADMIN." ORDER BY id DESC LIMIT $goto, $limit ");
																while($user = $db->fetch($query)){
																	$queryGroup = $db->select_query("SELECT * FROM ".TB_ADMIN_GROUP." WHERE id='".$user['level']."' ");
																	$group = $db->fetch($queryGroup);
																	?>
																	<tr>
																		<td width="44">
																			<a href="?name=admin&file=user&op=admin_edit&id=<?php echo $user['id'];?>"><img src="images/icon/edit.gif" border="0" alt="แก้ไข" ></a>
																			<a href="javascript:Confirm('?name=admin&file=user&op=admin_del&id=<?php echo $user['id'];?>','คุณมั่นใจในการลบชื่อผู้ใช้ : <?php echo $user['username'];?>');"><img src="images/icon/trash.gif"  border="0" alt="ลบ" ></a>
																		</td>
																		<td><?php echo $user['username'];?></td>
																		<td ><?php echo $user['name'];?></td>
																		<td ><?php echo $user['email'];?></td>
																		<td ><?php echo $group['name'];?></td>
																		<td  align="center" width="40"><input type="checkbox" name="list[]" value="<?php echo $user['id'];?>"></td>
																	</tr>
																	<TR>
															...
															while ($grouptxt = $db->fetch($query))
															{
																echo "<LI><B>".$grouptxt['name']." : </B>".$grouptxt['description']."</LI>";
															}
															$db->closedb ();

														}
														else if($_GET['op'] == "admin_add" AND ( isset($_GET['action']) && $_GET['action'] == "add" ) ){
															//////////////////////////////////////////// กรณีเพิ่ม User Admin Database
															$ProcessOutput = false;
															if(CheckLevel($_SESSION['admin_user'],$_GET['op'])){
																$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
																//ตรวจสอบมี user นี้หรือยัง
																$query = $db->select_query("SELECT id FROM ".TB_ADMIN." WHERE username='".$_POST['USERNAME']."' ");
																$adminRows = $db->rows($query);
																$db->closedb ();
																if($adminRows){
																	$ProcessOutput = "<BR><BR>";
																						...
																						<INPUT TYPE="text" NAME="NAME" size="40"><BR>
																							<B>Email :</B><BR>
																								<INPUT TYPE="text" NAME="EMAIL" size="40"><BR>
																									<B>Level :</B><BR>
																									...
																									else if($_GET['op'] == "admin_edit" AND ( isset($_GET['action']) && $_GET['action'] == "edit" ) ){
																										...
																										if(CheckLevel($_SESSION['admin_user'],$_GET['op'])){
																											...
																											$admin = $db->rows($query);
																											$db->closedb ();
																											if($admin AND ($_POST['USERNAME'] != $_POST['USERNAME_OLD'])){
																												$ProcessOutput = "<BR><BR>";
																												$ProcessOutput .= "<CENTER><IMG SRC=\"images/icon/notview.gif\" BORDER=\"0\"><BR><BR>";
																												$ProcessOutput .= "<FONT COLOR=\"#336600\"><B>ชื่อผู้ดูแลระบบ : ".$_POST['USERNAME']." มีในระบบแล้วไม่สามารถเพิ่มได้</B></FONT><BR><BR>";
																												$ProcessOutput .= "<A HREF=\"javascript:history.go(-1);\"><B>กลับไปแก้ไข</B></A>";
																										...
																										if(CheckLevel($_SESSION['admin_user'],$_GET['op'])){
																											...
																											$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
																											$query = $db->select_query("SELECT * FROM ".TB_ADMIN." WHERE id='".$_GET['id']."' ");
																											$admin = $db->fetch($query);
																											$db->closedb ();
																											//ไม่ให้อัพเดทตัวเอง
																											$Readonly = '';
																											if($_SESSION['admin_user'] == $admin['username']){
																												$Readonly = " readonly ";
																											}
																											?>
																											<FORM METHOD=POST ACTION="?name=admin&file=user&op=admin_edit&action=edit&id=<?=$_GET['id'];?>">
																												<B>ชื่อผู้ใช้ :</B><BR>
																													<INPUT TYPE="text" NAME="USERNAME" size="40" VALUE="<?=$admin['username'];?>"><BR><INPUT TYPE="hidden" NAME="USERNAME_OLD" VALUE="<?=$admin['username'];?>">
																												<B>รหัสผ่าน :</B><BR>
																													<INPUT TYPE="password" NAME="PASSWORD" size="40" VALUE="" <?=$Readonly;?>><BR>
																												<B>ชื่อ - นามสกุล :</B><BR>
																													<INPUT TYPE="text" NAME="NAME" size="40" VALUE="<?=$admin['name'];?>"><BR>
																												<B>Email :</B><BR>
																													<INPUT TYPE="text" NAME="EMAIL" size="40" VALUE="<?=$admin['email'];?>"><BR>
																												<B>Level :</B><BR>
																													<SELECT NAME="LEVEL">
																														<?php
																														$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
																														$queryGroup = $db->select_query("SELECT * FROM ".TB_ADMIN_GROUP." ORDER BY id ");
																														while ($groups = $db->fetch($queryGroup))
																														{
																															echo "<option value=\"".$groups['id']."\" ";
																															if($groups['id'] == $admin['level']){echo " Selected";};
																															echo ">".$groups['name']."</option>";
																														}
																														$db->closedb ();
																														?>
																													</SELECT>
																												<BR><BR>
																												<INPUT TYPE="submit" value=" แก้ไขผู้ดูแลระบบ ">
																												<INPUT TYPE="hidden" NAME="oldpass" value="<?=$admin['password'];?>">
																											</FORM>
																										<?php
																										}else{
																											//กรณีไม่ผ่าน
																										...
																										echo $ProcessOutput ;
																																				...
																																				else if($_GET['op'] == "admin_del" AND ( isset($_GET['action']) && $_GET['action'] == "multidel" ) ){
																																					$ProcessOutput = false;
																																					...
																																					if(CheckLevel($_SESSION['admin_user'],$_GET['op'])){
																																						//ดึงค่าของผู้ดูแลระบบออกมา
																																						$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
																																						$query = $db->select_query("SELECT * FROM ".TB_ADMIN." WHERE username='".$_SESSION['admin_user']."' ");
																																						$admin = $db->fetch($query);
																																						$db->closedb ();
																																						?>
																																						<FORM METHOD=POST ACTION="?name=admin&file=user&op=minepass_edit&action=edit">
																																							<B>ชื่อผู้ใช้ :</B><BR>
																																								<INPUT TYPE="text" NAME="USERNAME" size="40" VALUE="<?=$admin['username'];?>" readonly style="color=#FF0000;"><BR>
																																									<B>รหัสผ่าน :</B><BR>
																																											...
																																											<B>ชื่อ - นามสกุล :</B><BR>
																																												<INPUT TYPE="text" NAME="NAME" size="40" VALUE="<?=$admin['name'];?>"><BR>
																																													<B>Email :</B><BR>
																																														<INPUT TYPE="text" NAME="EMAIL" size="40" VALUE="<?=$admin['email'];?>"><BR>
																																															<BR><BR>
																																																<INPUT TYPE="submit" value=" แก้ไขข้อมูลส่วนตัว "><INPUT TYPE="hidden" NAME="oldpass" value="<?=$admin['password'];?>">
																																																</FORM>
																																																<?php
																																															}else{
																																																//กรณีไม่ผ่าน
