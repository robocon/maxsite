...
<?php 
	...
	if(CheckLevel($_SESSION['admin_user'],$_GET[op])){
		...
		if ((($FILE['type']!="image/jpg") AND ($FILE['type']!="image/jpeg") AND ($FILE['type']!="image/pjpeg")) AND $FILE['size']){
			...
			@copy ($FILE['tmp_name'] , "knowledgeicon/".$arr[knowledge][post_date].".jpg" );
			$original_image = "knowledgeicon/".$arr[knowledge][post_date].".jpg" ;
			$desired_width = _IKNOW_W ;
			$desired_height = _IKNOW_H ;
			$image = new hft_image($original_image);
			$image->resize($desired_width, $desired_height, '0');
			$image->output_resized("knowledgeicon/".$arr[knowledge][post_date].".jpg", "JPG");
		}
...
<?php 
	...
	if(CheckLevel($_SESSION['admin_user'],$_GET[op])){
		while(list($key, $value) = each ($_POST['list'])){
			...
			$db->closedb ();
			@unlink("knowledgedata/".$arr[knowledge][post_date].".txt");
			@unlink("knowledgeicon/".$arr[knowledge][post_date].".jpg");
		}
		$ProcessOutput .= "<BR><BR>";
		$ProcessOutput .= "<CENTER><A HREF=\"?name=admin&file=main\"><IMG SRC=\"images/icon/login-welcome.gif\" BORDER=\"0\"></A><BR><BR>";
