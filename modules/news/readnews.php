					...
					&nbsp;&nbsp;<IMG SRC="images/menu/textmenu_news.gif" BORDER="0"><BR><BR>
						<TABLE width="700" align=center cellSpacing=0 cellPadding=0 border=0>
							...
							if(!$item['id']){
								...
								$FileNewsTopic = "newsdata/".$item['post_date'].".txt";
								...
								$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
								$query = "UPDATE ".TB_NEWS." SET pageview = pageview+1 WHERE id = '".$_GET['id']."' ";
								mysql_query ( $query ) or sql_error ( "db-query",mysql_error() );
								//ชื่อหมวดหมู่
								$query = $db->select_query("SELECT * FROM ".TB_NEWS_CAT." WHERE id='".$item['category']."' ");
								$category = $db->fetch($query);
								$db->closedb ();
								?>
								<TR>
									<TD>
										<B><FONT COLOR="#990000"><?=$category['category_name'];?><BR><?=$item['topic'];?></FONT></B><BR>
													...
													<TR>
														<TD>
															<BR>
																<B><FONT COLOR="#990000"><?=$category['category_name'];?> 5 อันดับล่าสุด</B></FONT><BR><BR>
																	...
																	$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
																	$query = $db->select_query("SELECT * FROM ".TB_NEWS." WHERE category='".$category['id']."' ORDER BY id DESC LIMIT 5 ");
																	$cat_rows = $db->rows($query);
																	if(!$cat_rows){
														...
														<BR><BR>
															...
															if($enableComment){
																...
																//Check Comment
																$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
																$query = $db->select_query("SELECT * FROM ".TB_NEWS_COMMENT." WHERE news_id='".$item['id']."' ORDER BY id ");
																$count=0;
