					...
					&nbsp;&nbsp;<IMG SRC="images/menu/textmenu_knowledge2.gif" BORDER="0"><BR><BR>
						<TABLE width="700" align=center cellSpacing=0 cellPadding=0 border=0>
							...
							if(!$item['id']){
								...
								<TR>
									<TD>
										<B><FONT COLOR="#990000"><?=$category['category_name'];?><BR><?=$item['topic'];?></FONT></B>
											<BR>
												...
												if( isset($_SESSION['admin_user']) ){
													...
													?>
													<a href="?name=admin&file=knowledge&op=article_edit&id=<?php echo $item['id'];?>"><img src="images/admin/edit.gif" border="0" alt="แก้ไข" ></a>
													<a href="javascript:Confirm('?name=admin&file=knowledge&op=article_del&id=<?php echo $item['id'];?>&prefix=<?php echo $item['post_date'];?>','คุณมั่นใจในการลบหัวข้อนี้ ?');"><img src="images/admin/trash.gif"  border="0" alt="ลบ" ></a>
													<?php
												}
												?>
												<BR><BR>
												...
												<TD>
													<BR>
														...
														<TR>
															<TD>
																<BR>
																	<B><FONT COLOR="#990000"><?=$category['category_name'];?> 5 อันดับล่าสุด</B></FONT><BR><BR>
																		<?php
																		//แสดงสาระน่ารู้ 5 อันดับล่าสุดของหมวดหมู่
																		$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
																		$query = $db->select_query("SELECT * FROM ".TB_KNOWLEDGE." WHERE category='".$category['id']."' ORDER BY id DESC LIMIT 5 ");
																		$knowledge_rows = $db->rows($query);
																		if(!$knowledge_rows){
																			echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ไม่มีรายการในหมวดหมู่นี้ ";
																		}
																		while($knowledge = $db->fetch($query)){
																			?>
																			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<IMG SRC="images/icon/suggest.gif" BORDER="0" ALIGN="absmiddle"> <B><A HREF="?name=knowledge&file=readknowledge&id=<?=$knowledge['id'];?>" target="_blank"><?=$knowledge['topic'];?></A></B> <?= ThaiTimeConvert($knowledge['post_date'],"","");?><BR>
																				<?php
																			}
																			$db->closedb ();
																			?>
															...
															<BR><BR>
																...
																if( $enable_comment > 0 ){
																	...
																	//Check Comment
																	$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
																	$query = $db->select_query("SELECT * FROM ".TB_KNOWLEDGE_COMMENT." WHERE knowledge_id='".$item['id']."' ORDER BY id ");
																	$count=0;
																		...
																		<TABLE cellSpacing=5 cellPadding=0 width=480 border=0 align="center" class="tablecomment">
																			<TR>
																				<TD><B><FONT COLOR="#990000">ความคิดเห็นที่ <?=$count;?></FONT></B>
																					<?php if( isset($_SESSION['admin_user']) ){echo " <A HREF=\"?name=knowledge&file=delete_comment&id=".$_GET['id']."&comment=".$comment['id']."\"><IMG SRC=\"images/admin/trash.gif\" WIDTH=\"20\" HEIGHT=\"20\" BORDER=\"0\" ALIGN=\"absmiddle\"></A>";};?>
