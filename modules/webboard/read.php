...
?>
				...
				<TR>
					...
					?>
					<?=(CHANGE_EMOTICON(BBCODE($VIEWBOARD['detail'])));?>
					<BR><BR>
...
$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
$query = $db->select_query("SELECT * FROM ".TB_WEBBOARD_COMMENT." WHERE topic_id = '".$_GET['id']."' ORDER BY id ");
$count=0;
...
<BR>
			...
			<TR>
				...
				<?php if( isset($_SESSION['admin_user']) ){echo " <A HREF=\"javascript:Confirm('?name=webboard&file=delete_comment&id=".$_GET['id']."&comment=".$comment['id']."','คุณมั่นใจในการลบความคิดเห็นนี้ ?');\"><IMG SRC=\"images/admin/trash.gif\" WIDTH=\"20\" HEIGHT=\"20\" BORDER=\"0\" ALIGN=\"absmiddle\"></A>";};?>
...
//Show Picture
if($comment['picture']){
	$postpicupload = @getimagesize ("webboard_upload/".$comment['picture']."");
	if ( $postpicupload[0] > _WEBBOARD_LIMIT_PICWIDTH ) {
		$PicUpload = "<BR><CENTER><img src='webboard_upload/".$comment['picture']."' width='"._WEBBOARD_LIMIT_PICWIDTH."' border='0' ><BR></CENTER><BR>";
		}else{
		$PicUpload = "<BR><CENTER><img src='webboard_upload/".$comment['picture']."' border='0' ><BR></CENTER><BR>";
		}
	echo $PicUpload ;
}
?>
<?=(CHANGE_EMOTICON(BBCODE($comment['detail'])));?></TD>
			</TR>
			<TR>
				<TD height="1" class="dotline"></TD>
			</TR>
			<TR>
				<TD><B><FONT COLOR="#990000">โดย : </FONT></B> <?=$comment['post_name'];?> &nbsp;&nbsp; <FONT COLOR="#990000"><B>ไอพี : </B></FONT><?=$comment['ip_address'];?>
				</TD>
			</TR>
			</TABLE>
			<BR>
